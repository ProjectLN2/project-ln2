...
export default function Home() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-black text-white flex items-center justify-center">
      ...
      {/* OVERLAY plus léger */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(0,229,255,0.12),transparent_40%),linear-gradient(180deg,rgba(0,0,0,0.55),rgba(2,5,10,0.75),rgba(0,0,0,0.6))]" />
      ...
      <section className="relative z-10 px-6 text-center max-w-4xl">
        ...
        <div className="mx-auto mt-6 mb-8 flex items-center justify-center">
          <Image
            ...
            height={240}
            priority
            className="drop-shadow-[0_0_60px_rgba(0,229,255,0.75)]"
          />
        </div>
        ...
        <div className="mt-10 flex justify-center">
          <a
            href={discordLink}
            target="_blank"
            className="inline-flex items-center gap-2 rounded-2xl bg-cyan-300 text-black px-8 py-4 font-black hover:bg-cyan-200 transition shadow-xl shadow-cyan-500/30"
          >
            <Users size={20} />
            Join Discord
          </a>
...
function Info({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.05] px-5 py-4 backdrop-blur flex items-center justify-center gap-2 text-slate-300">
